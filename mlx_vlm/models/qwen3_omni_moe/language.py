...
class Qwen3VLMoEModel(nn.Module):
    ...
    def __call__(
        ...
        for layer_idx, (layer, c) in enumerate(zip(self.layers, cache)):
            ...
            # Early exit: the Talker only conditions on one layer's hidden state
            # (accept_hidden_layer). Returning here is byte-identical to
            # output_hidden_states[early_exit_layer + 1] but skips the remaining
            # layers + final norm + lm_head — halving the redundant thinker forward.
...
class LanguageModel(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # (no norm, no lm_head). Surface it directly; logits are not needed.
